...
The cluster-level ``boot_type`` default is a creation-time seed: every new
instance gets the resolved cluster value pinned as its own instance-level
setting at ``gnt-instance add``, and later cluster-level changes (via
``gnt-cluster modify -H kvm:boot_type=...``) affect only instances created
...
(``--with-ovmf-code-template`` / ``--with-ovmf-vars-template``); both
templates can be overridden per instance with the ``ovmf_code`` /
``ovmf_vars`` hvparams to pin an alternative OVMF build (use e.g. a
secure-boot pair). The firmware code is therefore pinned at
