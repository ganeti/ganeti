...
The cluster-level ``boot_type`` is a **creation-time seed**, not an
inherited default. At ``gnt-instance add``, if the op carries no explicit
``boot_type``, the resolved cluster value is *pinned* into
``instance.hvparams`` as a literal instance-level override; after creation
the instance never reads the cluster value again. ``gnt-cluster modify -H
kvm:boot_type=X`` is a pure config write: it prints a warning that it does
not change any existing instance and points at the per-instance path.
...
- ``cfgupgrade`` (``UpgradeCluster`` / ``UpgradeInstances``): the on-disk
  ...
  **every** KVM instance is pinned from its *effective* ``kernel_path``
  (instance override if present, else the cluster KVM ``kernel_path``).
  This inverts the old "don't freeze inheritance" rule: under the seed
  model there is no inheritance left to freeze, and an instance without a
  pinned value would have nothing to inherit. A matching
  ``DowngradeBootType`` strips ``boot_type``, ``ovmf_code`` and
  ``ovmf_vars`` for downgrades to 3.1.
- ``objects.Instance.UpgradeConfig``: fills defaults on every config load
  but sees one instance in isolation (instance overrides only), so it
  handles only the explicit-override case and stays idempotent; the
  no-override case is cfgupgrade's job, not "preserved inheritance".
...
size stays a fixed constant. The region geometry is therefore **frozen at
seed time**: a future re-seed must read the superblock and reject a
template larger than its recorded region. The layout logic lives in an
I/O-free module (``hv_kvm/firmware.py``) usable both master-side (to size
the disk) and node-side (to seed and to expose the regions).
...
Two templates seed the regions. Both are cluster-wide defaults that can be
overridden per instance at seed time, mirroring their symmetric lifecycle:
each is read exactly once, when the firmware disk is created and seeded
(instance ``add`` or ``gnt-instance modify -H boot_type=uefi``).
...
- ``--with-ovmf-vars-template`` (default ``/usr/share/OVMF/OVMF_VARS.fd``)
  doubles as the cluster-wide default for the per-instance ``ovmf_vars``
  hvparam, symmetric to ``ovmf_code``. A per-instance vars override is
  useful to pick a matching variant template at seed time (e.g. a
  secure-boot pair); later changes are no-ops until the future re-seed
  command exists.

Both follow the existing ``kvmKernel``/``kvmPath`` pattern (configure
value → ``AutoConf`` → Haskell constant → generated Python constant).
The ``ovmf_code``/``ovmf_vars`` pair must come from the same OVMF build;
this is an operator invariant, as Ganeti cannot verify it master-side.
...
- **Live migration** - ``_OpenInstanceDisks`` / ``CheckDiskConsistency`` /
  this is already covered by the existing code with the regular live
  migration procedure.
...
- **Switching ``boot_type``** is a disk lifecycle transition, modeled on
  the instance-communication NIC DDM. Only an **explicit instance-level**
  ``gnt-instance modify -H boot_type=uefi`` (instance stopped) creates
  and seeds a firmware disk (with node resource locks, as for a
  template change). A modify that leaves the resolved ``boot_type``
  unchanged is a no-op for the disk lifecycle. Switching **away** keeps
  the disk in place, inert (it holds the precious NVRAM); it can be
  removed explicitly later.
- **Start, failover and move never create disks.** An instance that
  *resolves* to ``uefi`` without a firmware disk fails their prerequisite
  check with the recovery command
  (``gnt-instance modify -H boot_type=uefi``, instance stopped). Live
  migration is exempt: a booted instance already has its disk; the
  ``HypervisorError`` in the hypervisor remains as a config-drift
  backstop.
- **Cluster-level default.** The cluster KVM ``boot_type`` is a
  creation-time seed: ``gnt-cluster modify -H kvm:boot_type=uefi`` is a
  pure config change that only affects instances created afterwards and
  warns that existing instances are untouched. Since every instance is
  pinned at creation (or at upgrade), no instance ever *resolves* to
  ``uefi`` via inheritance, so no migration machinery is needed.
...
- **``ovmf-instance update``** - operator-triggered re-seed of the **code**
  region; ``ovmf_code`` is already an hvparam to make this possible.
  Re-seeding the **vars** region is explicitly opt-in (a separate
  ``--reseed-vars`` style flag), never automatic: the vars region holds
  the precious per-instance NVRAM, and rewriting it destroys
  boot entries, keys and other UEFI state. ``ovmf_vars`` exists as an
  hvparam so such a command can resolve the template to seed from.
  Any re-seed must read the firmware superblock and reject a template
  larger than the region recorded at creation time - the region geometry
  is frozen at seed time (the disk itself has a fixed size).
