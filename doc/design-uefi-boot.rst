...
- **Switching ``boot_type``** is a disk lifecycle transition, modeled on
  ...
  template change). An **inherited** uefi (cluster default) is a no-op
  for the disk lifecycle. Switching **away** keeps the disk in place,
  inert (it holds the precious NVRAM); it can be removed explicitly
  later.
- **Start, failover and move never create disks.** An instance that
  *resolves* to ``uefi`` without a firmware disk fails their prerequisite
  ...
  backstop.
- **Cluster-level default.** ``gnt-cluster modify -H kvm:boot_type=uefi``
  is a pure config change. While KVM instances without an instance-level
  ``boot_type`` override and without a firmware disk exist, it is
  refused; ``--force`` applies it and submits a follow-up
  ``gnt-instance modify -H boot_type=uefi`` job for every affected
  **stopped** instance (creating and seeding its disk via the
  explicit-switch path). Running/offline instances are warned about and
  remain covered by the start guard. Migrated instances end up with an
  explicit - pinned - instance-level ``boot_type=uefi``.
