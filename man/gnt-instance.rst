...
boot\_type
    ...
    uefi
        ...
        is configured (affects RBD and Gluster storage backends).
    ...
    stopped) creates and seeds the firmware disk; a modify that leaves the
    resolved ``boot_type`` unchanged is a no-op. Switching away from
    ``uefi`` keeps the disk in place but inert; it can be removed
    explicitly later.

    **start**, **failover** and **move** refuse to run for an instance
    that resolves to ``uefi`` (instance setting or start-time override)
    but has no firmware disk; while stopped, run
